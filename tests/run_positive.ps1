...
$files = Get-ChildItem -Path (Join-Path $PSScriptRoot "positive") -Recurse -Filter *.csec |
...
if ($failed -gt 0) {
    ...
    foreach ($result in $results | Where-Object { -not $_.Passed }) {
        ...
        $highlights = $result.Output | Where-Object {
            $_ -match "Parsing failed|Parser Error|Type error|Type checking found|Unable to read input file|Failed to create ExecutionEngine|Timed out"
        } | Select-Object -First 3

        if ($highlights.Count -eq 0) {
            $highlights = $result.Output | Select-Object -First 3
        }
